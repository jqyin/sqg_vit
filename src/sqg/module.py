...
class PLModule(LightningModule):
    ...
    def __init__(
        ...
        self.net = net
        self.prof = FlopsProfiler(self.net)
        self.prof_step = 1
        self.tic = self.toc = 0
        self.warmup_step = 50
        self.bench_step = 100
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        if batch_idx == self.prof_step and self.global_rank == 0:
            ...
            #print(f"step:{batch_idx}, start profiling")
        
        if batch_idx == self.warmup_step and self.global_rank == 0:
            torch.cuda.synchronize()
            self.tic = time.time()
        if batch_idx == self.warmup_step+self.bench_step and self.global_rank == 0:
            torch.cuda.synchronize()
            self.toc = time.time()
            spi = (self.toc - self.tic)/self.bench_step
        ...
        if batch_idx == self.prof_step and self.global_rank == 0:
            ...
            self.flop = self.prof.get_total_flops()
            self.prof.print_model_profile(profile_step=self.prof_step, output_file="log.flops")
