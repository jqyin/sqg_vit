...
# define likelihood score
    ...
    def state_update_normalized(self,x_input,state_target_input, obs_input):
        ...
        ## convert state_target_input to float32 precision
        state_target_input = state_target_input.astype(np.float32) 

        x_state = torch.tensor(x_input,device=self.device)
        state_target = torch.tensor(state_target_input,device=self.device)
