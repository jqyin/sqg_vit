...
## Module for ViT and EnSF ensemble used ONLY for initial step for comparison purpose
...
class PLModule(LightningModule):
    ...
    def __init__(
        ...
        print('--- Mode 1: EnSF at initialization only')
        print('--- Followed by pure ViT auto-regressive predictions')
        ...
        self.bool_enable_ensf = False
        self.bool_enable_ensf_at_initialization_only = True
        self.bool_add_jump_noise = False
    ...
    def predict_step(self, batch: Any, batch_idx: int):
        ...
        if batch_idx == 0: ## t=0 
            _, _, lead_times, variables, out_variables = batch

            if self.bool_enable_ensf_at_initialization_only:
                ...
                print('SQG EnSF initial only, T: {}, rmse: {} '.format(batch_idx, np.sqrt(pverr_a.mean())))
                self.rmse_ensf.append(np.sqrt(pverr_a.mean()))
                
                if self.bool_save_preds:
                    self.pred_np_list.append(pvensmean_a)
                    self.pv_truth_list.append(self.pv_truth[batch_idx])
                    self.batch_idx_list.append(batch_idx)

                ### run ML model prediction for the mean of the 20 ensemble members
                ...
                x = torch.tensor(pvensmean_a, dtype=torch.float32)
        ...
        elif batch_idx >0 and batch_idx <=301: ## in the example in the paper, we tested 300 steps for demonstration
            _, _, lead_times, variables, out_variables = batch
    ...
    def gather_predictions(self):
        ...
        print('--- Running the gather_predictions function and saving results')
        if self.bool_enable_ensf:
            ...
            print('--- Saved the rmse_ensf_initial_only to CSV: {}'.format(self.coupled_prediction_output_path+'/rmse_ensf_initial_only.csv'))
            ...
            print('--- Saved the rmse_no_ensf to CSV: {}'.format(self.coupled_prediction_output_path+'/rmse_no_ensf.csv'))
            ### stack all numpy arrays along the first dimension
            stacked_numpy_preds = np.stack(self.pred_np_list, axis=0)
            ...
            # save the pv_truth_list
            stacked_pv_truth_np = np.stack(self.pv_truth_list, axis=0)
            # convert the <class 'numpy.ma.core.MaskedArray'> to <class 'numpy.ndarray'> before saving
            stacked_pv_truth_np = stacked_pv_truth_np.data
